...
def video_loader(video_dir_path, frame_indices, image_loader, dataset='pingpong_dataset'):
    video = []
...
class pingpong_dataset(data.Dataset):
    ...
    def __init__(self,
        ...
        file_path = [os.path.join(annotation_path, item) for item in os.listdir(annotation_path)]

        # if opt.demo_mode:
        # if is for demo to prepare dataset, just the first dataset is ok
            ...
            with open(file, mode='r') as f:
                ...
                for video_key in meta_data[0].keys():
                    for index, clip in enumerate(meta_data[0][video_key]):
                        if len(clip['action_catagory']) == 3 and int(clip['end_time']) > int(clip['start_time']):
        ...
        for index, item in enumerate(set([item['action_catagory'] for item in self.data])):
            self.idx_to_class[index] = item
            self.class_to_idx[item] = index

        for index, item in enumerate(self.data):
            self.data[index]['frame_indices'] = list(range(int(item['start_time']), int(item['end_time']) + 1))
            self.data[index].pop('start_time', None)
            self.data[index].pop('end_time', None)
            self.data[index]['action_catagory'] = self.class_to_idx[item['action_catagory']]
    ...
    def __getitem__(self, index):
        ...
        # meta_data ='file_index: {}; duration: {}~{}'.format(path, frame_indices[0], frame_indices[-1])
        return clip, target
...
class demo_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        meta_data ='{};{};{};{}'.format(path, frame_indices[0], frame_indices[-1], self.data[index]['action_catagory'])

        # meta_data = {'file_index': path, 'duration': [int(frame_indices[0]), int(frame_indices[-1])], 'action': int(self.data[index]['action_catagory'])}

...
def plot_pose(pose, index, dir_path):
    ...
    def joint_color(j):
        ...
        if j in range(14, 17):
            _c = 5
        return colors[_c]
    # assert is a kind of debug info
    assert (pose.ndim == 2)
    assert (pose.shape[0] == 3)
    fig = plt.figure()
    # gca means get current axes... bad name
    # I think is like fig.addsubplpt
    ...
    # fig.savefig('../data/'+dir_path+'/'+str(index)+'.png')

...
class k_folder_cross_validation(object):
    ...
    def writedown_split(self, path, data, catagory):
        ...
        for i in range(len(data)):
            data[i]['frame_indices'] = [data[i]['frame_indices'][0], data[i]['frame_indices'][-1]]
            data[i]['type'] = catagory
        with open(path, mode='w') as f:
            json.dump(data, f)

    def __call__(self):
        start_index, end_index = int( len(self.dataset) / self.k * self.n_th_call ), int( len(self.dataset) / self.k * (self.n_th_call + 1) ) #the two pointer to define the validation set
        ...
        training_data.data = training_data.data[:start_index] + training_data.data[end_index:]


        self.n_th_call = (self.n_th_call + 1) / self.k # this is the n + 1 th to call this class
        ...
        self.writedown_split(path='../pingpong_dataset_train.json', data=training_data.data, catagory='train')
        self.writedown_split(path='../pingpong_dataset_validation.json', data=validation_data.data, catagory='validation')
